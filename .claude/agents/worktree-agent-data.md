...
- **Auxiliary scripts** that aren't the migration itself — for transforming/backfilling existing data, or for manual rollback when the tool doesn't generate one on its own — go **outside the worktree, in `SCRIPTS_DEST`** (`scripts/<project>/`, at the root of this orchestrator). Same criterion `worktree-agent-docs` uses with `docs/<project>/`: it's the second explicit exception to "an agent only writes in its own worktree," alongside the Orchestrator's own `.orchestrator/`.
